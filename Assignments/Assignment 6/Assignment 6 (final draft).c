...
void postOrderFree(BSTNode * root);
...
int main()
{
  ...
  // if number of players is 0, print 0 and return
  if (numPlayers == 0)
  {
    printf("\nExcitement is: %lld\n", excitement);
    return 0;
...
BSTNode * insertNode(BSTNode * root, int tableNum)
{
  ...
  // checks if val is less then curr node's value
  // if yes, go to the left
  if (tableNum < root->tableNum)
  {
    root->left = insertNode(root->left, tableNum);
  }

  // checks if val is greater then curr node's value
...
// frees the tree in a post order traversal
void postOrderFree(BSTNode * root)
{
  // checks if tree is empty OR if we hit an empty node (aka - base case)
  if (root == NULL)
  {
    return;
  }

  // recurse to left and then right recursively
